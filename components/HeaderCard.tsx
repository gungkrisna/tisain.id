...
const HeaderCard = ({
  ...
  return (
    <Link href={href ? href : ""} target={newTab ? "_blank" : ""} onClick={onClick}>
      <span
        className={`block bg-black rounded-md md:rounded-2xl overflow-hidden shadow-md transition-all duration-300 ease-in-out cursor-pointer relative ${shadowColor} hover:shadow-xl`}
        onClick={onClick}
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
          <h5 className="text-md md:text-xl font-medium leading-tight text-white">
            {title}
          </h5>
        </div>
      </span>
